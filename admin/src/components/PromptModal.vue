  ...
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click="$emit('close')">
    ...
    <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-2/3 shadow-lg rounded-md bg-white" @click.stop>
      ...
      <form @submit.prevent="handleSubmit" class="mt-6 space-y-6">
        ...
        <!-- 描述 -->
        <div>
          <label for="description" class="block text-sm font-medium text-admin-700 mb-2">
            描述
          </label>
          <input
            id="description"
        ...
        <!-- 分类 -->
        <div>
          <label for="category" class="block text-sm font-medium text-admin-700 mb-2">
            分类
          </label>
          <select
            id="category"
        ...
        <!-- 提示词内容 -->
        <div>
          <label for="content" class="block text-sm font-medium text-admin-700 mb-2">
          ...
          <p class="mt-1 text-sm text-admin-500">{{ form.content.length }} 字符</p>
        </div>

        <!-- 标签 -->
        <div>
          <label for="tags" class="block text-sm font-medium text-admin-700 mb-2">
            标签
          </label>
          <div class="space-y-2">
            <!-- 标签输入 -->
          ...
          <p class="mt-1 text-sm text-admin-500">用逗号或回车分隔多个标签</p>
        </div>

        <!-- 高级选项 -->
        <div class="border-t border-admin-200 pt-6">
          <h4 class="text-sm font-medium text-admin-900 mb-4">高级选项</h4>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="flex items-center">
              ...
              />
              <label for="isFavorite" class="ml-2 block text-sm text-admin-900">
                标记为收藏
              </label>
            </div>
            <div>
              <label for="usageCount" class="block text-sm font-medium text-admin-700 mb-1">
                初始使用次数
              </label>
              <input
                id="usageCount"
                v-model.number="form.usageCount"
                type="number"
                min="0"
                class="input"
                placeholder="0"
...
watch(() => props.prompt, (newPrompt) => {
  if (newPrompt) {
    form.value = {
      ...
      usageCount: newPrompt.usageCount || 0
    }
  } else {
    resetForm()
  }
